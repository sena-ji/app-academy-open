...
def filter_out(array, &prc)
  ...
  array.each { |elem| filtered << elem if !prc.call(elem) }
...
def first_index(array, &prc)
  array.each { |elem| return array.index(elem) if prc.call(elem) }
...
def filter_out!(array, &prc)
  array.delete_if { |elem| prc.call(elem) }
end
...
def chain_map(value, prc_array)
  prc_array.inject(value) { |acc, prc| prc.call(acc) }
end
...
def proctition_platinum(array, *prcs)
  ...
  prcs.each_index { |i| prc_hash[i + 1] = [] }
  ...
  words.each do |word|
    ...
    hash.each do |key_prc, val_prc|
      if key_prc.call(word)
        new_word = val_prc.call(new_word)
      end
    end
    new_words << new_word
  end
