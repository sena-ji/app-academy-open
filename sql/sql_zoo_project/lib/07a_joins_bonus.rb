...
def heart_tracks
  ...
  execute(<<-SQL)
    SELECT
      albums.title,
      COUNT(tracks.song) AS total_number_of_tracks
    FROM
      albums
    JOIN
      tracks ON albums.asin = tracks.album
...
def rock_superstars
  ...
  execute(<<-SQL)
    SELECT
      albums.artist,
      COUNT(DISTINCT albums.title) AS number_of_albums
    FROM
      albums
    JOIN
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style,
      SUM(number_of_tracks_per_album.price) / SUM(number_of_tracks_per_album.number_of_tracks) AS avg_price_per_track
    FROM
      styles
    JOIN (
      SELECT
      ...
      GROUP BY
        albums.asin
    ) AS number_of_tracks_per_album ON styles.album = number_of_tracks_per_album.asin
    GROUP BY
      styles.style
    ORDER BY
      SUM(number_of_tracks_per_album.price) / SUM(number_of_tracks_per_album.number_of_tracks) DESC,
      styles.style ASC
    LIMIT
      5
  SQL
